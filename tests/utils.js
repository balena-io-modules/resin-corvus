...
describe('.transformForMixpanel()', () => {
  it('should return undefined if given undefined', () => {
    chai.expect(utils.transformForMixpanel(undefined)).to.be.undefined;
  });
  ...
  it('should return flat object with start case keys if given nested object', () => {
    ...
    chai.expect(utils.transformForMixpanel(object)).to.deep.equal({
  ...
  it('should return an object with the key `value` if given `false`', () => {
    chai.expect(utils.transformForMixpanel(false)).to.deep.equal({
      Value: false
    });
  });

  it('should return an object with the key `value` if given `null`', () => {
    chai.expect(utils.transformForMixpanel(null)).to.deep.equal({
      Value: null
    });
  });

  it('should preserve environment variable', () => {
    chai.expect(utils.transformForMixpanel({
  ...
  it('should not modify arrays', () => {
    chai.expect(utils.transformForMixpanel([1, 2, {
      nested: 3
    }])).to.deep.equal([1, 2, {
      Nested: 3
    }]);
